...
EnvironmentManager* setupScenario()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        {
            // Agent 1
            std::vector<double> position1 = {double(55 + i * 10),  double(55 + j * 10)};
            std::vector<double> destination1 = {double(-75), double(-75)};
            environment->addAgent(QLatin1String("Agent") + QString::number((i*5 + j)*4 + 0), position1, destination1);

            // Agent 2
            std::vector<double> position2 = {double(-55 - i * 10),  double(55 + j * 10)};
            std::vector<double> destination2 = {double(75), double(-75)};
            environment->addAgent(QLatin1String("Agent") + QString::number((i*5 + j)*4+ 1), position2, destination2);

            // Agent 3
            std::vector<double> position3 = {double(55 + i * 10), double(-55 - j * 10)};
            std::vector<double> destination3 = {double(-75), double(75)};
            environment->addAgent(QLatin1String("Agent") + QString::number((i*5 + j)*4 + 2), position3, destination3);

            // Agent 4
            std::vector<double> position4 = {double(-55 - i * 10), double(-55 - j * 10)};
            std::vector<double> destination4 = {double(75), double(75)};
